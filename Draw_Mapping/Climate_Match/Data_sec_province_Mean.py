...
RESULT_DIR = "02_province_weather_heatmaps_Aver"  # 结果文件夹名称
PLOT_DIR = "02_province_weather_heatmaps_Aver"  # 热力图保存文件夹
...
# ===========================================================================
# 第三步：省级指标计算（修改核心：算术平均值）
# ===========================================================================
def assign_station_to_province(station_data, province_gdf):
    """将气象站点匹配到对应的省级行政区"""
    # 将站点数据转为GeoDataFrame
    ...
    station_with_province = gpd.sjoin(
        station_geo,
        province_gdf[["province_code", "province_name", "geometry"]],
        how="left",
        predicate="within"
    )

    # 移除冗余列
    station_with_province = station_with_province.drop(columns=["index_right", "geometry"])

    # 统计匹配结果
    matched_count = station_with_province["province_code"].notna().sum()
    total_count = len(station_with_province)
    print(f"✅ 站点匹配完成：{matched_count}/{total_count} 个站点匹配到省级行政区")

    return station_with_province


def calculate_province_average(station_stats, province_gdf):
    """按省级行政区计算站点指标的算术平均值"""
    print("\n===== 计算省级算术平均值 =====")

    # 1. 匹配站点到省级行政区
    station_with_province = assign_station_to_province(station_stats, province_gdf)

    # 2. 按省份+年份分组计算算术平均值
    province_stats = station_with_province.groupby(["province_code", "province_name", "year"]).agg(
        ...
        station_count=("STATION", "nunique")  # 记录每个省每年的站点数量
    ).reset_index()

    # 保留两位小数
    for metric in EXTREME_METRICS:
        province_stats[metric] = province_stats[metric].round(2)
...
def generate_province_panel_data(province_gdf, station_stats, result_dir):
    ...
    # 计算省级算术平均值
    province_panel_df = calculate_province_average(station_stats, province_gdf)
    ...
    if not province_panel_df.empty:
        ...
        total_provinces = len(province_panel_df["province_code"].unique())
        print(f"ℹ️ 有效省级记录数：{valid_provinces} 条（覆盖 {total_provinces} 个省）")
...
def create_province_heatmap(year, metric, province_gdf, province_panel_df, station_stats, plot_dir):
    ...
    # 图表样式设置
    ax.set_title(
        f"{year}年 中国省级{metric_alias}热力图（算术平均）",
