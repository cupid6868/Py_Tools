...
RESULT_DIR = "02_city_weather_heatmaps"  # 结果文件夹名称
PLOT_DIR = "02_city_weather_heatmaps"  # 热力图保存文件夹
...
def load_city_boundaries(shp_path):
    ...
    # 提取中心点经纬度
    gdf["city_lat"] = gdf["centroid"].y
    gdf["city_lon"] = gdf["centroid"].x

    # 整理结果
    city_gdf = gdf.rename(columns={
        code_field: "city_code",
        name_field: "city_name"
    })[["city_code", "city_name", "geometry", "city_lat", "city_lon"]]
...
# ===========================================================================
# 第三步：反距离加权（IDW）插值计算市级指标
# ===========================================================================
def idw_interpolation(city_gdf, station_data, year, power=2, max_distance=1000000):
    """
    反距离加权插值计算市级指标
    :param city_gdf: 市级边界GeoDataFrame
    :param station_data: 单年份站点数据
    :param year: 目标年份
    :param power: 距离幂次
    :param max_distance: 最大插值距离（米）
    :return: 市级指标DataFrame
    """
    city_results = []
    ...
    # 遍历每个市
    for _, city_row in tqdm(city_gdf.iterrows(), desc=f"IDW插值-{year}年", total=len(city_gdf)):
        city_code = city_row["city_code"]
        city_name = city_row["city_name"]
        city_lat = city_row["city_lat"]
        city_lon = city_row["city_lon"]
        ...
        for _, station_row in year_station_data.iterrows():
            ...
            # 计算距离（米）
            dist = haversine_distance(city_lat, city_lon, station_lat, station_lon)
        ...
        # 计算IDW加权平均
        city_values = {"city_code": city_code, "city_name": city_name, "year": year}
        ...
        if distances:
            ...
            for metric in EXTREME_METRICS:
                if values[metric]:
                    ...
                    city_values[metric] = round(weighted_avg, 2)
                else:
                    city_values[metric] = np.nan
        else:
            # 无有效站点，标记为NaN
            for metric in EXTREME_METRICS:
                city_values[metric] = np.nan

        city_results.append(city_values)
...
def generate_city_panel_data(city_gdf, station_stats, result_dir):
    ...
    for year in range(TARGET_YEARS[0], TARGET_YEARS[1] + 1):
        city_year_data = idw_interpolation(city_gdf, station_stats, year, IDW_POWER, IDW_MAX_DISTANCE)
        if not city_year_data.empty:
            all_city_data.append(city_year_data)

    # 合并所有年份数据
    if all_city_data:
        city_panel_df = pd.concat(all_city_data, ignore_index=True)
        # 排序
        city_panel_df = city_panel_df.sort_values(["year", "city_code"]).reset_index(drop=True)
        ...
        total_cities = city_panel_df.shape[0]
        print(f"ℹ️ 有效市级记录数：{valid_cities}/{total_cities}")
...
def create_city_heatmap(year, metric, city_gdf, city_panel_df, station_stats, plot_dir):
    ...
    # 图表样式设置
    ax.set_title(
        f"{year}年 中国市域{metric_alias}热力图（IDW插值）",
