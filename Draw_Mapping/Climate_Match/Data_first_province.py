...
SHP_PATH = r"D:\Data_2\2023年省级\省级.shp"  # 省级边界SHP文件
TARGET_START_YEAR = 2010  # 起始年份
TARGET_END_YEAR = 2024  # 结束年份
RESULT_DIR = "01_province_weather_plots"  # 省级结果文件夹
PLOT_DIR = "01_province_weather_plots"  # 省级热力图保存文件夹
# IDW参数
IDW_POWER = 2  # 距离幂次（值越大，近站点权重越高）
MIN_STATIONS = 1  # 每个省份至少需要的站点数
...
# SHP文件字段映射（适配常见省级SHP字段）
PROVINCE_FIELD_MAP = {
    "code": ["CODE", "ADMINCODE", "省级码", "行政代码", "ID", "GID"],
    "name": ["NAME", "省", "名称", "省份名", "CNAME", "ProvinceName"]
}
# 图表样式配置（适配省级尺度）
PLOT_CONFIG = {
    "figsize": (18, 12),  # 省级图表尺寸更大
    "dpi": 300,  # 分辨率
    "cmap": "RdYlBu_r",  # 配色方案（coolwarm/viridis/YlOrRd可选）
    "station_size": 8,  # 站点标记大小（省级尺度适当放大）
    "station_color": "#333333",  # 站点颜色（深灰更柔和）
    "station_alpha": 0.4,  # 站点透明度
    "station_show": True,  # 是否显示站点
    "station_filter_distance": 10000,  # 过滤密集站点（米，省级尺度阈值更高）
...
def filter_dense_stations(station_df, min_distance=10000):
    """过滤密集站点：保留距离大于阈值的站点，避免重叠（省级尺度阈值更高）"""
    if len(station_df) <= 1:
        return station_df
...
def load_province_boundaries(shp_path):
    ...
    # 提取中心点经纬度
    gdf["prov_lat"] = gdf["centroid"].y
    gdf["prov_lon"] = gdf["centroid"].x

    # 匹配代码/名称字段
    code_field = next((f for f in PROVINCE_FIELD_MAP["code"] if f in gdf.columns), None)
    if code_field is None:
        code_field = "prov_code_temp"
        gdf[code_field] = gdf.index.astype(str)

    name_field = next((f for f in PROVINCE_FIELD_MAP["name"] if f in gdf.columns), None)
    if name_field is None:
        name_field = "prov_name_temp"
        gdf[name_field] = [f"省份_{i}" for i in gdf.index]

    # 整理结果
    province_gdf = gdf.rename(columns={
        code_field: "prov_code",
        name_field: "prov_name"
    })[["prov_code", "prov_name", "geometry", "prov_lat", "prov_lon"]]

    print(f"✅ 加载完成：共{len(province_gdf)}个省/直辖市/自治区")
    return province_gdf


# ===========================================================================
# 第三步：IDW插值生成省级面板数据（核心逻辑）
# ===========================================================================
def idw_interpolate_province(prov_centroid, stations_df, power=2):
    """
    反距离权重插值：计算省级行政中心的气象指标
    核心逻辑：距离越近的气象站，对目标省份的影响权重越大
    :param prov_centroid: 省级中心点 (lat, lon)
    :param stations_df: 年度站点数据
    :param power: 距离幂次（值越大，近站点权重占比越高）
    :return: 各指标插值结果
    """
    if len(stations_df) < MIN_STATIONS:
        return {metric: np.nan for metric in METRICS.keys()}

    prov_lat, prov_lon = prov_centroid
    # 计算所有站点到该省中心点的距离
    distances = np.array([
        haversine_distance(prov_lat, prov_lon, row["纬度(°)"], row["经度(°)"])
        for _, row in stations_df.iterrows()
    ])
    distances = np.where(distances == 0, 1e-6, distances)  # 避免除以0

    # 计算权重：1/距离^power（距离越近，权重越大）
...
def aggregate_to_province(station_df, province_gdf, result_dir):
    ...
    for year in tqdm(range(TARGET_START_YEAR, TARGET_END_YEAR + 1), desc="📍 处理年度数据"):
        ...
        for _, province in province_gdf.iterrows():
            idw_result = idw_interpolate_province(
                (province["prov_lat"], province["prov_lon"]),
                year_stations,
                power=IDW_POWER
            )
            # 组装结果
            province_row = {
                "年份": year,
                "省份代码": province["prov_code"],
                "省份名称": province["prov_name"],
                "省份纬度(°)": round(province["prov_lat"], 6),
                "省份经度(°)": round(province["prov_lon"], 6)
            }
            province_row.update(idw_result)
            province_results.append(province_row)

    # 保存结果
    province_df = pd.DataFrame(province_results)
    province_df = province_df.sort_values(["年份", "省份代码"]).reset_index(drop=True)
    output_path = os.path.join(result_dir, "02_province_weather_panel_2010_2024.csv")
    province_df.to_csv(output_path, index=False, encoding="utf-8-sig")
    print(f"✅ 省级面板数据（IDW插值）已保存：{os.path.abspath(output_path)}")
    return province_df
...
def create_province_heatmap(year, metric, province_gdf, province_df, station_df, plot_dir):
    ...
    province_gdf_with_data.plot(
        ...
        edgecolor='white', linewidth=0.8  # 省级边界加白色描边，更清晰
    )
    ...
    # 图表样式
    ax.set_title(
        f"{year}年 中国省级{METRICS[metric]}热力图（IDW插值）",
    ...
    # 保存图片
    plot_filename = sanitize_filename(f"{year}年_{metric_alias}_IDW插值.png")
...
def main():
    try:
        ...
        # 3. 加载省级边界
        province_gdf = load_province_boundaries(SHP_PATH)
