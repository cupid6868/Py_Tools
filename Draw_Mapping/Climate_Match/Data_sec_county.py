...
COUNTY_SHP_PATH = r"D:\Data_2\2023年县级\县级.shp"  # 县级边界SHP文件路径
BASELINE_YEARS = (1981, 2010)  # 气候基准期
TARGET_YEARS = (2010, 2024)  # 统计极端天数的目标期
RESULT_DIR = "02_county_weather_heatmaps"  # 结果文件夹名称
PLOT_DIR = "02_county_weather_heatmaps"  # 热力图保存文件夹

# IDW插值参数
IDW_POWER = 2  # 距离倒数的幂次（常用2）
IDW_MAX_DISTANCE = 1000000  # 最大插值距离（米，1000公里）

# SHP字段映射（根据你的县级SHP文件调整）
COUNTY_FIELD_MAP = {
    "code": ["县代码", "CODE", "ADMINCODE", "ID"],  # 县级代码字段
    "name": ["县名称", "NAME", "CNAME", "地名"]  # 县级名称字段
}
...
FILES_CONFIG = {
    ...
    "county_panel": "04_county_weather_panel_data.csv"
}
...
def load_county_boundaries(shp_path):
    ...
    # 匹配县级代码和名称字段
    code_field = next((f for f in COUNTY_FIELD_MAP["code"] if f in gdf.columns), None)
    if code_field is None:
        code_field = "county_code_temp"
        gdf[code_field] = gdf.index.astype(str)

    name_field = next((f for f in COUNTY_FIELD_MAP["name"] if f in gdf.columns), None)
    if name_field is None:
        name_field = "county_name_temp"
        gdf[name_field] = [f"县_{i}" for i in gdf.index]
    ...
    # 统一代码为字符串类型
    county_gdf["county_code"] = county_gdf["county_code"].astype(str)
...
# ===========================================================================
# 第三步：反距离加权（IDW）插值计算县级指标
# ===========================================================================
def idw_interpolation(county_gdf, station_data, year, power=2, max_distance=1000000):
    """
    反距离加权插值计算县级指标
    :param county_gdf: 县级边界GeoDataFrame
    :param station_data: 单年份站点数据
    :param year: 目标年份
    :param power: 距离幂次
    :param max_distance: 最大插值距离（米）
    :return: 县级指标DataFrame
    """
    county_results = []
    ...
    # 遍历每个县
    for _, county_row in tqdm(county_gdf.iterrows(), desc=f"IDW插值-{year}年", total=len(county_gdf)):
        ...
        for _, station_row in year_station_data.iterrows():
            ...
            # 计算距离（米）
            dist = haversine_distance(county_lat, county_lon, station_lat, station_lon)
        ...
        # 计算IDW加权平均
        county_values = {"county_code": county_code, "county_name": county_name, "year": year}
        ...
        if distances:
            ...
            for metric in EXTREME_METRICS:
                if values[metric]:
                    ...
                    county_values[metric] = round(weighted_avg, 2)
                else:
                    county_values[metric] = np.nan
        else:
            # 无有效站点，标记为NaN
            for metric in EXTREME_METRICS:
                county_values[metric] = np.nan

        county_results.append(county_values)
...
def generate_county_panel_data(county_gdf, station_stats, result_dir):
    """生成县级气象面板数据"""
    print("\n===== 生成县级气象面板数据 =====")
    all_county_data = []
    ...
    for year in range(TARGET_YEARS[0], TARGET_YEARS[1] + 1):
        county_year_data = idw_interpolation(county_gdf, station_stats, year, IDW_POWER, IDW_MAX_DISTANCE)
        if not county_year_data.empty:
            all_county_data.append(county_year_data)

    # 合并所有年份数据
    if all_county_data:
        county_panel_df = pd.concat(all_county_data, ignore_index=True)
        # 排序
        county_panel_df = county_panel_df.sort_values(["year", "county_code"]).reset_index(drop=True)

        # 保存面板数据
        output_path = os.path.join(result_dir, FILES_CONFIG["county_panel"])
        county_panel_df.to_csv(output_path, index=False, encoding="utf-8-sig")
        print(f"✅ 县级面板数据已保存：{output_path}")

        # 统计信息
        valid_counties = county_panel_df.dropna(subset=EXTREME_METRICS).shape[0]
        total_counties = county_panel_df.shape[0]
        print(f"ℹ️ 有效县级记录数：{valid_counties}/{total_counties}")

        return county_panel_df
    else:
        print("❌ 未生成任何县级数据")
        return pd.DataFrame()
...
# 第四步：生成县域热力图（含气象站标注）
# ===========================================================================
def create_county_heatmap(year, metric, county_gdf, county_panel_df, station_stats, plot_dir):
    """生成单年份单指标的县域热力图，标注气象站"""
    # 创建文件夹
    ...
    # 筛选数据
    year_county_df = county_panel_df[county_panel_df["year"] == year].copy()
    year_station_df = station_stats[station_stats["year"] == year].copy()

    # 合并边界与指标数据
    county_gdf["county_code_str"] = county_gdf["county_code"].astype(str)
    year_county_df["county_code_str"] = year_county_df["county_code"].astype(str)

    merged_gdf = county_gdf.merge(
        year_county_df[["county_code_str", metric]],
        left_on="county_code_str",
        right_on="county_code_str",
        how="left"
    ...
    # 图表样式设置
    ax.set_title(
        f"{year}年 中国县域{metric_alias}热力图（IDW插值）",
...
def generate_all_heatmaps(county_gdf, county_panel_df, station_stats, plot_dir):
    """生成所有年份和指标的热力图"""
    print("\n===== 生成县域热力图 =====")
    ...
    with tqdm(total=total_plots, desc="生成热力图") as pbar:
        for year in range(TARGET_YEARS[0], TARGET_YEARS[1] + 1):
            for metric in EXTREME_METRICS:
                try:
                    create_county_heatmap(year, metric, county_gdf, county_panel_df, station_stats, plot_dir)
                    pbar.update(1)
...
def main():
    try:
        ...
        # 3. 县级数据处理
        print("\n===== 步骤4：加载县级边界数据 =====")
        county_gdf = load_county_boundaries(COUNTY_SHP_PATH)

        print("\n===== 步骤5：IDW插值生成县级面板数据 =====")
        county_panel_df = generate_county_panel_data(county_gdf, extreme_stats, result_dir)

        # 4. 生成热力图
        if not county_panel_df.empty:
            generate_all_heatmaps(county_gdf, county_panel_df, extreme_stats, plot_dir)

        # 输出最终统计信息
        print("\n===== 📊 最终结果统计 =====")
        print(f"📅 时间范围：{TARGET_YEARS[0]}-{TARGET_YEARS[1]} 年")
        print(f"📍 涉及站点数：{extreme_stats['STATION'].nunique()}")
        print(f"🏘️ 涉及县级行政区数：{county_gdf['county_code'].nunique()}")
        print(f"📈 有效县级记录数：{len(county_panel_df.dropna(subset=EXTREME_METRICS))}")
        print(f"\n✅ 所有任务完成！")
