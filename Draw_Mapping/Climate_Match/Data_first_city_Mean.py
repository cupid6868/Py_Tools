...
RESULT_DIR = "01_city_weather_plots_Aver"  # 地级结果文件夹
PLOT_DIR = "01_city_weather_plots_Aver"  # 地级热力图保存文件夹
...
def load_prefecture_boundaries(shp_path):
    ...
    if code_field is None:
        ...
        gdf[code_field] = gdf.index.astype(str)  # 直接生成字符串类型代码
    ...
    prefecture_gdf["pref_code"] = prefecture_gdf["pref_code"].astype(str)  # 统一为字符串

    print(f"✅ 加载完成：共{len(prefecture_gdf)}个地级市")
    return prefecture_gdf

# ===========================================================================
# 第三步：算术平均法生成地级面板数据（核心修改）
# ===========================================================================
def assign_station_to_prefecture(station_df, prefecture_gdf):
    """将每个站点匹配到所属的地级市"""
    # 将站点数据转为GeoDataFrame
    ...
    # 空间连接：匹配站点所属的地级市
    station_with_pref = gpd.sjoin(
        station_geo,
        prefecture_gdf[["pref_code", "pref_name", "geometry"]],
        how="left",
        predicate="within"
    )

    # 处理未匹配到地级市的站点（设为NaN）
    station_with_pref["pref_code"] = station_with_pref["pref_code"].fillna("未知")
    station_with_pref["pref_name"] = station_with_pref["pref_name"].fillna("未知")

    return station_with_pref


def aggregate_to_prefecture_by_average(station_df, prefecture_gdf, result_dir):
    """
    核心修改：算术平均法生成地级面板数据
    对同一地级市下所有站点的指标取算术平均值
    """
    print("\n===== 第三步：算术平均法生成地级气象面板数据 =====")
    # 1. 将站点匹配到地级市
    station_with_pref = assign_station_to_prefecture(station_df, prefecture_gdf)

    # 2. 按年份+地级市分组计算算术平均值
    prefecture_results = []
    for year in tqdm(range(TARGET_START_YEAR, TARGET_END_YEAR + 1), desc="📍 处理年度数据"):
        year_stations = station_with_pref[station_with_pref["年份"] == year].copy()
        if year_stations.empty:
            continue

        # 按地级市分组计算均值
        pref_group = year_stations.groupby(["pref_code", "pref_name"]).agg({
            "纬度(°)": "mean",  # 地级市所有站点的平均纬度
            "经度(°)": "mean",  # 地级市所有站点的平均经度
            **{metric: "mean" for metric in METRICS.keys()}  # 各指标算术平均
        }).reset_index()

        # 整理字段名和格式
        for _, row in pref_group.iterrows():
            prefecture_row = {
                "年份": year,
                "地级市代码": row["pref_code"],
                "地级市名称": row["pref_name"],
                "地级市纬度(°)": round(row["纬度(°)"], 6),
                "地级市经度(°)": round(row["经度(°)"], 6)
            }
            # 填充各指标均值（保留2位小数）
            for metric in METRICS.keys():
                prefecture_row[metric] = round(row[metric], 2) if not pd.isna(row[metric]) else np.nan
    ...
    print(f"✅ 地级面板数据（算术平均）已保存：{os.path.abspath(output_path)}")

    # 输出统计信息
    unknown_stations = station_with_pref[station_with_pref["pref_name"] == "未知"]["气象站编号"].nunique()
    print(f"ℹ️ 未匹配到地级市的站点数：{unknown_stations}")
...
def main():
    try:
        ...
        # 4. 生成地级面板数据（算术平均法）
        prefecture_df = aggregate_to_prefecture_by_average(station_df, prefecture_gdf, result_dir)
