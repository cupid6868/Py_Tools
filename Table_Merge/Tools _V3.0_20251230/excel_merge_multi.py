...
class ExcelMergerPro:
    ...
    def __init__(self, root):
        ...
        # 快速通知系统
        self.fast_notify_queue = Queue()

        # 启动快速通知监听器
        self.start_fast_notify_listener()
    ...
    def _run_merge_process(self, config):
        ...
        try:
            ...
            if save_success:
                # 立即发送快速通知
                self.fast_notify_queue.put(("success", f"合并结果已保存到：\n{config['output_path']}"))
                # 同时发送常规结果，但UI更新更快
                self.result_queue.put({"type": "save_success_fast", "path": config["output_path"], "df": main_df})
            else:
    ...
    def _save_output_file(self, df, path, output_format):
        ...
        try:
            ...
            # 使用快速通知系统发送错误
            self.fast_notify_queue.put(("error", f"保存{output_format.upper()}文件失败：{str(e)}"))
            return False
    ...
    def handle_result(self, msg):
        ...
        elif msg["type"] == "save_success_fast":
            # 快速更新UI，不等待弹窗
            self.is_running = False
    ...
    def start_fast_notify_listener(self):
        """启动快速通知监听器"""

        def listen():
            while True:
                try:
                    if not self.fast_notify_queue.empty():
                        msg_type, msg_content = self.fast_notify_queue.get()
                        self.root.after(0, lambda t=msg_type, c=msg_content: self.show_fast_notify(t, c))
    ...
    def show_fast_notify(self, msg_type, msg_content):
        """显示快速通知"""
        if msg_type == "success":
            messagebox.showinfo("成功", msg_content)
        elif msg_type == "error":
